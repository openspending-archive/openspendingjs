(function() {
  ...
  $.fn.insertAt = function(idx, element) {
    ...
    if (idx < 0) {
      idx = Math.max(0, lastIdx + 1 + idx);
    }
    this.append(element);
    if (idx < lastIdx) {
      this.children().eq(idx).before(this.children().last());
    }
    return this;
  };
  Delegator = (function() {
  ...
  util = {
    ...
    flattenForm: function(data, form) {
      ...
      for (k in _ref) {
        v = _ref[k];
        el = form.find("[name=\"" + k + "\"]");
        _results.push(el.attr('type') === 'checkbox' ? el.prop('checked', true) : el.val(v));
      }
      return _results;
    },
    validId: function(s) {
  ...
  AddDimensionWidget = (function() {
    ...
    AddDimensionWidget.prototype.onTypeChange = function(e) {
      ...
      if (suggestion in this.modelEditor.data) {
        return false;
      }
  ...
  UniquesWidget = (function() {
    ...
    UniquesWidget.prototype.onShow = function(e) {
      ...
      list.empty();
      add = __bind(function(dim) {
        var checked, txt;
        checked = this.modelEditor.data[dim]['key'];
    ...
    UniquesWidget.prototype.onSetUniquesClick = function(e) {
      ...
      if (!list.find(':checked').length) {
        ...
        list.find('input').each(__bind(function(idx, elt) {
          var dim, tgt;
          dim = $(elt).attr('name');
          tgt = $("[name=\"" + dim + "[key]\"]");
  ...
  DimensionsWidget = (function() {
    ...
    DimensionsWidget.prototype.removeDimension = function(name) {
      ...
      if (idx !== null) {
        this.widgets.splice(idx, 1)[0].element.remove();
      }

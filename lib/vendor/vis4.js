...
var vis4utils = function() {};

vis4utils.log = function(s) {
	//if (window.console !== null) console.log(s);
};

vis4utils.str2time = function(s) {
	var p = s.split(".");
	return Math.round(Date.UTC(2000+p[0], p[1]-1, p[2])*0.001);
};

vis4utils.round = function(val, prec) {
	var d = Math.pow(10, prec);
	return Math.round(val*d)/d;
};
vis4utils.formatNumber_ksep = '.';
vis4utils.formatNumber_dsep = ',';
vis4utils.formatNumber = function(nr, round) {
	if (nr > 999999 && round) return (''+vis4utils.round(nr/1000000,1)).replace(".",vis4utils.formatNumber_dsep)+"&nbsp;Mio";
	nr = ''+nr;
	var out = '', c=0;
	for (var i=nr.length-1;i>=0;i--) {
		if (c > 0 && c < nr.length && c%3==0) out = vis4utils.formatNumber_ksep + out;
		out = nr[i] + out;
		c++;
	}
	return out;
};
vis4utils.parseTSV = function(raw, asObject) {
	var lines = raw.split("\n");
	var data = [];
	var props;
	var l;
...
vis4utils.map = function(arr, idCol) {
	var map = {};
...
vis4utils.DelayedTask = function(delay, scope, func, args) {
	this.func = func;
	this.scope = scope;
	this.args = args;
	this.running = true;
	this.timer = window.setTimeout(this.run.bind(this), delay);
};
vis4utils.DelayedTask.prototype.run = function() {
	this.func.apply(this.scope, this.args);
	this.running = false;
};
vis4utils.DelayedTask.prototype.cancel = function() {
...
var vis4loadingItem = function(url, id, type, ldr) { 
	...
	this.processContent = function(content) {
		if (this.type == 'tsv') this.data = vis4utils.parseTSV(content, true);

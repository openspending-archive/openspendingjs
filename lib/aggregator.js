...
(function($) {
  var defaultConfig = {
    ...
    'callback': function(tree) { console.log('Loaded tree', tree); }
  };
  ...
  OpenSpending.Aggregator = function(customConfig) {
	var d = {
		...
		processEntry: function(entry, nodes) {
            var parent = nodes.root,
                level = 0,
                drilldown,
                drilldowns = this.config.drilldows,
                current,
                node,
                node_template;
        ...
        addBreakdown: function(node, entry) {
            ...
            node_template = this.stoNode(breakdown_value);
	...
	console.log('Hello world says aggregator');
	d.getTree();
};
